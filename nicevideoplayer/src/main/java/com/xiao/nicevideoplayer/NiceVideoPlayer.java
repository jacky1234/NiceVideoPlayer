...
import com.xiao.nicevideoplayer.floating.Floating;
...
public class NiceVideoPlayer extends FrameLayout
    ...
    private void init() {
        ...
        mFloating = new Floating(NiceUtil.scanForActivity(mContext));
    }
    ...
    private IMediaPlayer.OnErrorListener mOnErrorListener
        ...
        public boolean onError(IMediaPlayer mp, int what, int extra) {
            handlerErrorInternal(mp, what, extra);
            return true;
        }
    };

    private void handlerErrorInternal(IMediaPlayer mp, int what, int extra) {
        // 直播流播放时去调用mediaPlayer.getDuration会导致-38和-2147483648错误，忽略该错误
        // mediaPlayer 本地路径不存的时候，报 what == 1 && extra == -2147483648 错误
        if (what != -38 && what != -2147483648 && extra != -38 && extra != -2147483648 || (what == 1 && extra == -2147483648)) {
            if (what == 1 && extra == -2147483648) {
                LogUtil.d("onError ——> STATE_ERROR ———— 本地文件不存在");
            }
...
//        contentView.addView(mContainer, params);
        ...
        final int originW = getWidth();
        final int originH = getHeight();
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                final float ration = Math.min(0.4f, animatedFraction);
                final float w = (1 - ration) * originW;
                final float h = (1 - ration) * originH;
                LogUtil.d("animatedFraction:" + animatedFraction + ",ration=" + ration + ",w=" + w + ",h=" + h);
    ...
    public void enterTinyWidonwSmooth() {
        enterTinyWindowSmooth(true);
    }

    private void enterTinyWindowSmooth(boolean flag) {
        if (mCurrentMode == MODE_TINY_WINDOW) return;

        if (flag) {
            mFloating.start(mContainer, new Floating.OnFloatingDelegate() {
                @Override
